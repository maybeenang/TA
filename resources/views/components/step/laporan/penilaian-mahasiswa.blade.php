<div class="space-y-2">
    ...
    <div class="space-y-4 rounded-md border border-l-8 border-zinc-300 border-l-red-500 p-2">
        ...
        <div class="">
            ...
            <form action="{{route('tenaga-pengajar.laporan.importPenilaian', $laporan)}}" method="post"
                enctype="multipart/form-data" class="inline" x-ref="importForm">
                @csrf
                @method('PUT')
                <input x-on:change="$refs.importForm.submit()" type="file" name="file" id="file" class="hidden"
                    x-ref="importInput" accept=".xlsx,.xls">
            </form>
